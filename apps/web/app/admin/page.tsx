...
export default function AdminConsole() {
  ...
  const commandList = [
    { label: t('palette.goDashboard'), action: () => { setActiveTab('dashboard'); setCommandPaletteOpen(false); } },
    { label: t('palette.goUsers'), action: () => { setActiveTab('users'); setCommandPaletteOpen(false); } },
    { label: t('palette.goOrgs'), action: () => { setActiveTab('organizations'); setCommandPaletteOpen(false); } },
    { label: t('palette.goNfc'), action: () => { setActiveTab('nfc-qr'); setCommandPaletteOpen(false); } },
    { label: t('palette.goJobs'), action: () => { setActiveTab('jobs'); setCommandPaletteOpen(false); } },
    { label: t('palette.goFlags'), action: () => { setActiveTab('flags'); setCommandPaletteOpen(false); } },
    { label: t('palette.goSecurity'), action: () => { setActiveTab('security'); setCommandPaletteOpen(false); } },
    { label: t('palette.goLogs'), action: () => { setActiveTab('logs'); setCommandPaletteOpen(false); } },
    { label: t('palette.goDeveloper'), action: () => { setActiveTab('developer'); setCommandPaletteOpen(false); } },
    { label: t('palette.toggleTheme'), action: () => { toggleTheme(); setCommandPaletteOpen(false); } },
    { label: t('palette.refreshMetrics'), action: () => { loadData(); setCommandPaletteOpen(false); } },
    { label: t('palette.logout'), action: () => { localStorage.clear(); window.location.href = '/login'; } },
  ];
      ...
      <div data-theme={theme} className="min-h-screen bg-canvas text-ink flex items-center justify-center p-12">
        <div className="w-full max-w-lg space-y-6">
          <div className="flex items-center gap-3 justify-center mb-6">
            ...
            <span className="text-xl font-extrabold tracking-tight">{t('enterpriseConsole')}</span>
          </div>
    ...
    >
      ...
      <aside className="w-[280px] bg-surface border-e border-line flex flex-col shrink-0">
        ...
        <div className="p-6 border-b border-line flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9.5px] font-bold text-accent tracking-widest uppercase mt-0.5">{t('brand')}</span>
            </div>
          </div>
          <Badge variant="error" className="text-[9px] px-1.5 py-0.5">{t('platform')}</Badge>
        </div>

        {/* Navigation Items */}
        <nav className="flex-1 overflow-y-auto p-4 space-y-1.5">
          <div className="text-[10px] font-extrabold text-[hsl(var(--v-faint))] tracking-wider uppercase px-3 mb-2">{t('logs.operations')}</div>
          <button
          ...
          >
            <span className="flex items-center gap-3"><Icon name="gauge" size={15} /> {t('nav.dashboard')}</span>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="users" size={15} /> {t('nav.users')}</span>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="grid" size={15} /> {t('nav.organizations')}</span>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="tag" size={15} /> {t('nav.nfc-qr')}</span>
          </button>

          <div className="text-[10px] font-extrabold text-[hsl(var(--v-faint))] tracking-wider uppercase px-3 pt-6 mb-2">{t('logs.systems')}</div>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="inbox" size={15} /> {t('nav.jobs')}</span>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="lock" size={15} /> {t('nav.flags')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="shield" size={15} /> {t('nav.security')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="file-text" size={15} /> {t('nav.logs')}</span>
          </button>
          ...
          >
            <span className="flex items-center gap-3"><Icon name="gauge" size={15} /> {t('nav.developer')}</span>
          </button>
        </nav>
        ...
        <div className="p-4 border-t border-line bg-canvas/30 space-y-3">
          ...
          >
            <Icon name="arrow-left" size={14} /> {t('backToApp')}
          </button>
      ...
      <main className="flex-1 overflow-y-auto bg-canvas flex flex-col min-w-0">
        ...
        <header className="h-[70px] bg-surface border-b border-line px-8 flex items-center justify-between shrink-0 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span className="flex items-center gap-2"><Icon name="search" size={13} /> {t('header.searchCommand')}</span>
              <span className="text-[10px] font-mono border border-line bg-surface px-1.5 py-0.5 rounded-lg">Ctrl+K</span>
            </div>

            <Button size="sm" variant="outline" onClick={loadData}>
              <Icon name="refresh" size={12} className="mr-1.5" /> {t('header.refresh')}
            </Button>
          </div>
        </header>
        ...
        <div className="p-8 flex-1 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="hover:shadow-md transition-shadow">
                  <CardBody className="p-5 flex flex-col justify-between h-32">
                    <span className="text-[11.5px] font-extrabold tracking-wider text-muted uppercase">{t('kpi.totalUsers')}</span>
                    <div>
                      <div className="text-[28px] font-extrabold leading-none tracking-tight">{kpi.totals.users}</div>
                      <span className="text-[10px] text-muted mt-1.5 inline-block">{t('kpi.activeLabel')}: {kpi.totals.activeUsers}</span>
                    </div>
                  </CardBody>
                </Card>
                ...
                <Card className="hover:shadow-md transition-shadow">
                  <CardBody className="p-5 flex flex-col justify-between h-32">
                    <span className="text-[11.5px] font-extrabold tracking-wider text-muted uppercase">{t('sections.organizations')}</span>
                    <div>
                      <div className="text-[28px] font-extrabold leading-none tracking-tight">{kpi.totals.organizations}</div>
                      <span className="text-[10px] text-muted mt-1.5 inline-block">{t('kpi.cardsLabel')}: {kpi.totals.cards}</span>
                    </div>
                  </CardBody>
                </Card>
                ...
                <Card className="hover:shadow-md transition-shadow">
                  <CardBody className="p-5 flex flex-col justify-between h-32">
                    <span className="text-[11.5px] font-extrabold tracking-wider text-muted uppercase">{t('kpi.activeNfc')}</span>
                    <div>
                      <div className="text-[28px] font-extrabold leading-none tracking-tight">{kpi.totals.activeNfc}</div>
                      <span className="text-[10px] text-muted mt-1.5 inline-block">{t('kpi.registeredLabel')}: {kpi.totals.nfcDevices}</span>
                    </div>
                  </CardBody>
                </Card>
                ...
                <Card className="hover:shadow-md transition-shadow">
                  <CardBody className="p-5 flex flex-col justify-between h-32">
                    <span className="text-[11.5px] font-extrabold tracking-wider text-muted uppercase">{t('kpi.mrrArr')}</span>
                    <div>
                      <div dir="ltr" className="text-[28px] font-extrabold leading-none tracking-tight">${kpi.totals.mrr}</div>
                      <span dir="ltr" className="text-[10px] text-muted mt-1.5 inline-block">{t('kpi.arrLabel')}: ${kpi.totals.arr}</span>
                    </div>
                  </CardBody>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-2">
                  <CardHeader className="border-b border-line px-6 py-4">
                    <span className="text-[13.5px] font-extrabold">{t('composition.title')}</span>
                  </CardHeader>
                  <CardBody className="p-6">
                    {(() => {
                      const rows = [
                        { label: t('composition.users'), value: kpi.totals.users, color: '#2563eb' },
                        { label: t('composition.organizations'), value: kpi.totals.organizations, color: '#4f46e5' },
                        { label: t('composition.cards'), value: kpi.totals.cards, color: '#0ea5e9' },
                        { label: t('composition.publishedCards'), value: kpi.totals.publishedCards, color: '#10b981' },
                        { label: t('composition.nfcDevices'), value: kpi.totals.nfcDevices, color: '#ec4899' },
                        { label: t('composition.leads'), value: kpi.totals.leads, color: '#f59e0b' },
                      ];
                ...
                <Card>
                  <CardHeader className="border-b border-line px-6 py-4">
                    <span className="text-[13.5px] font-extrabold text-ink">{t('summary.revenueStorage')}</span>
                  </CardHeader>
                  <CardBody className="p-6 space-y-4 text-[12px]">
                    <div className="flex justify-between">
                      <span className="text-muted">{t('summary.mrr')}</span>
                      <span className="font-extrabold text-ink">${kpi.totals.mrr}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted">{t('summary.arr')}</span>
                      <span className="font-extrabold text-ink">${kpi.totals.arr}</span>
                    </div>
                    <div className="flex justify-between border-t border-line pt-4">
                      <span className="text-muted">{t('summary.dbSize')}</span>
                      <span className="font-extrabold text-ink">{kpi.totals.dbSize ?? '—'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted">{t('summary.views')}</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card className="bg-surface/50 border border-line">
                  <CardBody className="p-4 text-center">
                    ...
                    <span className="text-[11px] text-muted font-bold tracking-tight uppercase">{t('kpi.leads')}</span>
                  </CardBody>
                </Card>
                <Card className="bg-surface/50 border border-line">
                  <CardBody className="p-4 text-center">
                    <div className="text-[20px] font-black text-ink">{kpi.totals.profileViews}</div>
                    <span className="text-[11px] text-muted font-bold tracking-tight uppercase">{t('kpi.totalViews')}</span>
                  </CardBody>
                </Card>
                <Card className="bg-surface/50 border border-line">
                  <CardBody className="p-4 text-center">
                    <div className="text-[20px] font-black text-ink">{kpi.totals.publishedCards}</div>
                    <span className="text-[11px] text-muted font-bold tracking-tight uppercase">{t('kpi.publishedCards')}</span>
                  </CardBody>
                </Card>
                <Card className="bg-surface/50 border border-line">
                  <CardBody className="p-4 text-center">
                    <div className="text-[20px] font-black text-ink">{kpi.totals.profileViews}</div>
                    <span className="text-[11px] text-muted font-bold tracking-tight uppercase">{t('kpi.trackedEvents')}</span>
                  </CardBody>
                </Card>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 justify-between bg-surface p-4 rounded-xl border border-line">
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  ...
                  <Select
                    ...
                    className="w-40"
                  >
                    <option value="ALL">{t('flags.allUsers')}</option>
                    <option value="ACTIVE">{t('kpi.activeUsers')}</option>
                    <option value="SUSPENDED">{t('kpi.suspendedUsers')}</option>
                    <option value="ADMIN">{t('kpi.superAdmins')}</option>
                  </Select>
                </div>
                  ...
                  <table className="w-full border-collapse text-left text-[13px]">
                    <thead>
                      <tr className="border-b border-line bg-surface/50 font-extrabold text-muted text-[11px] uppercase tracking-wider">
                        <th className="p-4">{t('users.userAccount')}</th>
                        <th className="p-4">{t('users.orgsRoles')}</th>
                        <th className="p-4">{t('kpi.totalAssets')}</th>
                        <th className="p-4">{t('users.createdDate')}</th>
                        <th className="p-4">{t('users.privilege')}</th>
                        <th className="p-4 text-right">{t('logs.operational')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredUsers.length === 0 ? (
                        <tr>
                          <td colSpan={6} className="p-8 text-center text-muted">{t('users.empty')}</td>
                        </tr>
                      ) : (
                        filteredUsers.map((u) => (
                          <tr key={u.id} className="border-b border-line hover:bg-surface/30">
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-extrabold text-ink">{u.name || t('users.anonymous')}</div>
                            ...
                            <td className="p-4">
                              <div className="space-y-1">
                                ...
                                {u.organizations.length === 0 && <span className="text-muted italic text-[11px]">{t('users.noMembership')}</span>}
                              </div>
                            </td>
                            <td className="p-4">
                              <span className="font-bold text-ink">{u.cardsCount} {t('users.cardsCount')}</span>
                              <span className="text-muted text-[11px] block">{u.leadsCount} {t('users.leadsCount')}</span>
                            </td>
                            ...
                            <td className="p-4">
                              <div className="space-y-1">
                                <Badge variant={u.isSuperAdmin ? 'error' : 'neutral'} className="text-[10px]">
                                  {u.isSuperAdmin ? t('users.superAdmin') : t('users.standardUser')}
                                </Badge>
                            ...
                            <td className="p-4 text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <Button size="sm" variant="secondary" className="text-[11.5px]" onClick={() => handleToggleUserAdmin(u.id, u.isSuperAdmin)}>
                                  {u.isSuperAdmin ? t('users.revokeAdmin') : t('users.grantAdmin')}
                                </Button>
                              </div>
                            </td>
                          </tr>
                  ...
                  <Card key={o.id} className="hover:shadow-md transition-shadow">
                    ...
                    <CardBody className="p-6 space-y-4 text-[12.5px]">
                      ...
                      <div className="border-t border-line pt-4 flex gap-2 justify-end items-center flex-wrap">
                        ...
                        >
                          <option value="FREE">{t('orgs.upgradeFree')}</option>
                          <option value="PRO">{t('orgs.upgradePro')}</option>
                          <option value="BUSINESS">{t('orgs.upgradeBusiness')}</option>
                          <option value="ENTERPRISE">{t('orgs.upgradeEnterprise')}</option>
                        </Select>
          ...
          {activeTab === 'jobs' && (
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader className="border-b border-line px-6 py-4">
                  <span className="text-[13.5px] font-extrabold text-ink">{t('jobs.title')}</span>
                </CardHeader>
                <CardBody className="p-0">
                  <table className="w-full border-collapse text-left text-[13px]">
                    <thead>
          ...
          {activeTab === 'flags' && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              <Card>
                <CardHeader className="border-b border-line px-6 py-4">
                  <span className="text-[13.5px] font-extrabold text-ink">{t('flags.registerTitle')}</span>
                </CardHeader>
                <CardBody className="p-6 space-y-4">
                  <div>
                    <label className="block text-[11px] font-extrabold uppercase tracking-wider text-muted mb-1.5">{t('flags.flagName')}</label>
                    <Input placeholder={t('flags.keyPlaceholder')} className="w-full" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-extrabold uppercase tracking-wider text-muted mb-1.5">{t('flags.audience')}</label>
                    <Select className="w-full">
                      <option>{t('users.allPlatforms')}</option>
                      <option>{t('kpi.betaOptIns')}</option>
                      <option>{t('flags.enterpriseOnly')}</option>
                    </Select>
                  </div>
                  <div>
                    <label className="block text-[11px] font-extrabold uppercase tracking-wider text-muted mb-1.5">{t('flags.rolloutPct')}</label>
              ...
              <Card className="xl:col-span-2">
                <CardHeader className="border-b border-line px-6 py-4">
                  <span className="text-[13.5px] font-extrabold text-ink">{t('flags.title')}</span>
                </CardHeader>
                <CardBody className="p-0">
                  <table className="w-full border-collapse text-left text-[13px]">
                    <thead>
                      <tr className="border-b border-line bg-surface/50 font-extrabold text-muted text-[11px] uppercase tracking-wider">
                        <th className="p-4">{t('flags.flagKey')}</th>
                        <th className="p-4">{t('flags.audienceRule')}</th>
                        <th className="p-4">{t('flags.rollout')}</th>
                        <th className="p-4">{t('flags.gateState')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {flags.map((flag) => (
                        <tr key={flag.id} className="border-b border-line hover:bg-surface/30">
                          ...
                          <td className="p-4">
                            <span className="font-extrabold text-blue-600">{flag.rollout}% {t('flags.rolloutSuffix')}</span>
                          </td>
                          <td className="p-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[12px] font-bold ${flag.status ? 'text-emerald-500' : 'text-red-500'}`}>
                                {flag.status ? t('flags.enabled') : t('flags.disabled')}
                              </span>
                            </div>
                          </td>
                        </tr>
          ...
          {activeTab === 'security' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className="lg:col-span-2">
                <CardHeader className="border-b border-line px-6 py-4 flex items-center justify-between">
                  <span className="text-[13.5px] font-extrabold text-ink">{t('security.firewallTitle')}</span>
                  <Badge variant="error">{blockedIPs.length} {t('security.blockedIps')}</Badge>
                </CardHeader>
                <CardBody className="p-0">
                  <table className="w-full border-collapse text-left text-[13px]">
                    <thead>
                      <tr className="border-b border-line bg-surface/50 font-extrabold text-muted text-[11px] uppercase tracking-wider">
                        <th className="p-4">{t('security.blockedIp')}</th>
                        <th className="p-4">{t('security.reason')}</th>
                        <th className="p-4 text-right">{t('users.actions')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {blockedIPs.map((ip) => (
                        <tr key={ip} className="border-b border-line hover:bg-surface/30">
                          ...
                          <td className="p-4 text-muted">{t('security.failedSignins')}</td>
                          <td className="p-4 text-right">
              ...
              <Card>
                <CardBody className="p-0 overflow-x-auto">
                  <table className="w-full border-collapse text-left text-[12.5px]">
                    <thead>
                      <tr className="border-b border-line bg-surface/50 font-extrabold text-muted text-[10px] uppercase tracking-wider">
                        <th className="p-4">{t('logs.timestamp')}</th>
                        <th className="p-4">{t('logs.executor')}</th>
                        <th className="p-4">{t('logs.action')}</th>
                        <th className="p-4">{t('logs.targetType')}</th>
                        <th className="p-4">{t('logs.targetId')}</th>
                        <th className="p-4">{t('logs.metadata')}</th>
                      </tr>
                    </thead>
                    <tbody>
          ...
          {activeTab === 'developer' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                <CardHeader className="border-b border-line px-6 py-4">
                  <span className="text-[13.5px] font-extrabold text-ink">{t('developer.envVars')}</span>
                </CardHeader>
              ...
              <Card>
                <CardHeader className="border-b border-line px-6 py-4">
                  <span className="text-[13.5px] font-extrabold text-ink">{t('security.maintenanceTitle')}</span>
                </CardHeader>
                <CardBody className="p-6 space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="font-bold text-ink">{t('security.maintenanceMode')}</div>
                      <span className="text-[11px] text-muted block">{t('security.maintenanceHint')}</span>
                    </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="font-bold text-ink">{t('security.restrictSignups')}</div>
                      <span className="text-[11px] text-muted block">{t('security.restrictHint')}</span>
                    </div>
          ...
          <div className="w-full max-w-lg bg-surface border border-line rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="max-h-72 overflow-y-auto p-2 space-y-1">
              {filteredCommands.length === 0 ? (
                <div className="p-4 text-center text-muted text-[13px]">{t('palette.empty')}</div>
              ) : (
                filteredCommands.map((cmd, idx) => (
                  ...
                  >
                    <span>{cmd.label}</span>
                    <span className="text-[10px] text-muted">{t('palette.pressEnter')}</span>
                  </div>
                ))
              )}
            </div>
      ...
      {createUserModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-surface border border-line rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-line flex items-center justify-between">
              <h2 className="text-[16px] font-black text-ink">{t('users.createTitle')}</h2>
            ...
            <form onSubmit={handleCreateUser} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between border-t border-line/60 pt-4">
                <div>
                  <div className="text-[12.5px] font-bold text-ink">{t('security.grantSuperAdmin')}</div>
                  <span className="text-[10px] text-muted block">{t('security.grantHint')}</span>
                </div>
      ...
      {createOrgModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-surface border border-line rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-line flex items-center justify-between">
              <h2 className="text-[16px] font-black text-ink">{t('orgs.createTitle')}</h2>
            ...
            <form onSubmit={handleCreateOrganization} className="p-6 space-y-4">
              {createOrgStatus && (
                <Alert
                  ...
                  title={createOrgStatus.type === 'success' ? t('common.success') : t('common.error')}
              ...
              <div>
                ...
                >
                  ...
                  <option value="PRO">{t('tiers.pro')}</option>
                  <option value="BUSINESS">{t('tiers.business')}</option>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input
                      type="email"
                      required
                      placeholder={t('orgs.ownerEmailPlaceholder')}
                  ...
                  {ownerOption === 'new' && (
                    <>
                      <div>
                        ...
                        <Input
                          placeholder={t('orgs.ownerNamePlaceholder')}
      ...
      {changeOwnerModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-surface border border-line rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-line flex items-center justify-between">
              <div>
                <h2 className="text-[16px] font-black text-ink">{t('orgs.changeOwner')}</h2>
            ...
            <form onSubmit={handleChangeOrgOwner} className="p-6 space-y-4">
              {changeOwnerStatus && (
                <Alert
                  ...
                  title={changeOwnerStatus.type === 'success' ? t('common.success') : t('common.error')}
              ...
              <div>
                ...
                <Input
                  type="email"
                  required
                  placeholder={t('orgs.managerEmailPlaceholder')}
      ...
      {deleteOrgModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-surface border border-line rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-line flex items-center justify-between">
              <h2 className="text-[16px] font-black text-ink">{t('orgs.deleteTitle')}</h2>
            ...
            <div className="p-6 space-y-4">
              <p className="text-[13.5px] text-muted leading-relaxed">
                {/* The org name is bolded mid-sentence, so this needs Trans rather than t(). */}
                <Trans
                  i18nKey="orgs.deleteConfirm"
                  ns="admin"
                  values={{ name: deleteOrgName }}
                  components={{ 1: <span className="font-extrabold text-ink" /> }}
                />
              </p>
              <div className="flex gap-3 justify-end pt-4 border-t border-line/60">
