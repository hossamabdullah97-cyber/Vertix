...
const METRICS: Metric[] = [
  { key: 'VIEW', label: 'Profile Views', icon: 'eye', accentDot: '#2563eb' },
  { key: 'SAVE', label: 'Contact Saves', icon: 'user-plus', accentDot: '#22c55e' },
  { key: 'NFC_SCAN', label: 'NFC Taps', icon: 'sparkle', accentDot: '#f59e0b' },
  { key: 'SHARE', label: 'Shares', icon: 'qr', accentDot: '#0ea5e9' },
  { key: 'CLICK', label: 'Link Clicks', icon: 'link', accentDot: '#ec4899' },
];
  ...
  return (
    <div className="v-card p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-[13px] font-semibold tracking-tight">Engagement</h3>
        <span className="v-badge v-badge-neutral">All time</span>
      </div>
      {counts && total === 0 ? (
        <p className="rounded-xl border border-dashed border-line bg-canvas/40 px-3 py-4 text-center text-[11.5px] text-muted">
          No activity yet. Numbers appear here once people open this card&apos;s link.
        </p>
      ) : (
